...
Recent work has explored scaling compute at inference time. Adaptive Chain-of-Focus~\cite{adaptivecof} dynamically decides when to zoom. AdaZoom-GUI~\cite{adazoomgui} introduced adaptive zoom with instruction refinement. Training-Free Uncertainty Guidance~\cite{uncertaintyguidance} uses model uncertainty to select visual inputs. Reinforcement learning approaches such as GRPO-based visual grounding~\cite{guigrpo,deepseekr1}, GUI-G$^2$~\cite{guig2} with Gaussian rewards, and Holo2~\cite{holo2} have shown promise for improving localization through online RL. Alternative alignment methods like KTO~\cite{kto} offer simpler binary-feedback training. Our method can be viewed as a minimal form of test-time compute scaling: we run two models instead of one and select the better prediction.
