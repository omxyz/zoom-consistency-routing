...
Recent work has explored scaling compute at inference time. Adaptive Chain-of-Focus~\cite{adaptivecof} dynamically decides when to zoom. AdaZoom-GUI~\cite{adazoomgui} introduced adaptive zoom with instruction refinement. Training-Free Uncertainty Guidance~\cite{uncertaintyguidance} uses model uncertainty to select visual inputs. Reinforcement learning approaches such as GRPO-based visual grounding~\cite{guigrpo} and Holo2~\cite{holo2} have shown promise for improving localization through online RL. Our method can be viewed as a minimal form of test-time compute scaling: we run two models instead of one and select the better prediction.
